...
app.post('/api/analyze', analyzeLimiter, async (req, res) => {
  ...
  if (finalResponse) {
    ...
    const analysisPromise = (async () => {
      ...
      console.log(`[PROGRESS] Cloned ${repoUrl}. Running 6 analyzers...`);
      const [codeQuality, security, accessibility, seo, maintainability, scalability] = await Promise.all([
        ...
        analyzeSEO(tempDir), analyzeMaintainability(tempDir), analyzeScalability(tempDir)
      ]);
      
      return { result, codeQuality, security, accessibility, seo, maintainability, scalability };
    })();
    ...
    try {
      const { result, codeQuality, security, accessibility, seo, maintainability, scalability } = await Promise.race([analysisPromise, timeoutPromise]);
      const aggregated = aggregateScores({ codeQuality, security, accessibility, seo, maintainability, scalability });
